...
class PlugLunar {
     ...
     * @return string 节气
     */
    public function getJieQiPrev($year, $month, $day) {
        $res = $this->getJieQi($year, $month, $day);
        if ($res) {
            return $res;
        }
        $date = $year . $month . $day;
        for ($i = 1; $i <= 50; $i++) {
            $prevDate = date('Ymd', strtotime("$date -$i day"));
            $prevDateY = substr($prevDate, 0, 4);
            $prevDateM = substr($prevDate, 4, 2);
            $prevDateD = substr($prevDate, 6, 2);
            $res = $this->getJieQi($prevDateY, $prevDateM, $prevDateD);
            if ($res) {
                return $res;
            }
        }
